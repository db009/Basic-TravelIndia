...
if (!$conn) {
	...
	$query="Select * from bus where Origin='$origin'and Destination='$destination'";
	...
	if(mysqli_num_rows($resultValue)>0)
	{
		...
		{ 
	        ...
	        echo "<h3><font color='black'>Available Buses</font></h3>";
			echo "</td>";
			echo "<tr><th class='pp'>BusId</th>
                      <th class='pp'>Bus Name</th>
			...
			echo "<tr><td class='pp'>{$row['BusId']}</td>
			          <td class='pp'>{$row['Bus_Name']}</td>
			...
			echo "<td>";
			echo "<br>";
            echo "<button><a href='payment.php?travellers=".$travellers."& Bus_id=".$Bus_id."& Total_fare=".$Total_fare."'>Book Now</a></button>";
  		    echo "</td>";
			echo "</tr>";
