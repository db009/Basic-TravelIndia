...
$childA=$_POST["traveller_childA"];
$childB=$_POST["traveller_childB"];
...
if (!$conn) {
	...
	$query="Select * from flight where Origin='$Origin'and Destination='$Destination'";
	...
	if(mysqli_num_rows($resultValue)>0)
	{
		...
		{ 
			...
			$traveller = $Adults + $childA + $childB;
	        $Total_Fare = $row['Fare'] * $traveller;
			
	        echo "<tr>";
			echo "<td style='text-align:center;'>";
	        echo "<h3><font color='black'>Available Flights</font></h3>";
			echo "</td>";
	        echo "<tr><th class='pp'>FlightId</th>
                      <th class='pp'>Flight Name</th>
			...
			echo "<tr><td class='pp'>{$row['FlightId']}</td>
			          <td class='pp'>{$row['Flight_Name']}</td>
			...
			echo "<td>";
			echo "<br>";
            echo "<button><a href='payment2.php?traveller=".$traveller."& Total_Fare=".$Total_Fare."&  Flight_id=".$Flight_id."'>Book Now</a></button>";
  		    echo "</td>";
			echo "</tr>";
